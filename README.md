...
Anthropic `backend: vendor_cli` is implemented for Claude Code delegation. Other future-facing seams such as Codex `backend: vendor_cli` and `credential_source: command` still fail explicitly instead of silently emulating API-key behavior.
