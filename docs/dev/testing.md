...
`pytest` is configured in `pyproject.toml` with `pythonpath=["src"]` and `testpaths=["tests"]`, and every run records code coverage for `kb_librarian` (`--cov`, branch coverage on). `make test` keeps pytest color enabled for result markers and progress percentages, then calls `make coverage` to print the saved report for all measured files. `make coverage` can also reprint that report without rerunning tests. Transient products (pytest cache, coverage data) are written under `build/`, and bytecode writes are disabled to keep the source tree clean; `make clean` removes `build/`, `dist/`, `*.egg-info`, and any stray `__pycache__`.
